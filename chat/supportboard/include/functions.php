...
function sb_is_agent($user = false) {
    ...
    return in_array(sb_isset($user, 'user_type'), ['agent', 'admin', 'bot']);
}
...
function sb_get_conversations($pagination = 0, $status_code = 0, $routing = false, $routing_unassigned = false, $department = false, $source = false) {
    $status_code = $status_code == 0 ? 'AND C.status_code <> 3 AND C.status_code <> 4' : 'AND C.status_code = ' . sb_db_escape($status_code);
    $result = sb_db_get('SELECT A.*, B.user_type as message_user_type, C.title FROM sb_messages A, sb_users B, sb_conversations C WHERE B.id = A.user_id ' . $status_code . ' AND C.id = A.conversation_id' . ($source ? ' AND C.source = "' . sb_db_escape($source) . '"' : '') . (sb_get_agent_department() !== false ? ' AND C.department = ' . sb_get_agent_department() : ($department ? ' AND C.department = ' . sb_db_escape($department, true) : '')) . sb_routing_db($routing, $routing_unassigned, 'C') . ' AND A.id IN (SELECT max(id) FROM sb_messages WHERE message <> "" OR attachments <> "" GROUP BY conversation_id) GROUP BY conversation_id ORDER BY C.status_code DESC, A.id DESC LIMIT ' . (intval(sb_db_escape($pagination, true)) * 100) . ',100', false);
    if (isset($result) && is_array($result)) {
